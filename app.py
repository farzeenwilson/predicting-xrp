...
#setting up page
st.set_page_config(page_title="Predicting Ripple Price", page_icon="📈", layout="wide")
st.title("Predicting Ripple (XRP) Price")
st.markdown("""
This dashboard pulls the latest live data for **Ripple (XRP)**, calculates key technical indicators, 
...
def load_data():
    ...
    xrp['SMA_30'] = xrp['XRP_Close'].rolling(window=30).mean()
    
    ema_12 = xrp['XRP_Close'].ewm(span=12, adjust=False).mean()
    ema_26 = xrp['XRP_Close'].ewm(span=26, adjust=False).mean()
    xrp['MACD'] = ema_12 - ema_26
...
def train_and_predict(df):
    ...
    #LSTM for fast rendering on webapp
    model = Sequential()
...
with st.spinner('Fetching live market data and updating neural network...'):
    df = load_data()
...
#filtering df based on slider
df_display = df.tail(days_to_show)

#displaying prediction metric
# Fixed the yfinance MultiIndex bug here by forcing floats
